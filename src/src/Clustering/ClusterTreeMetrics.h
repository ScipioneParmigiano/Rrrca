...
// information.                                                               //
////////////////////////////////////////////////////////////////////////////////
#ifndef RRCA_CLUSTERING_CLUSTERTREEMETRICS_H_
#define RRCA_CLUSTERING_CLUSTERTREEMETRICS_H_
...
void clusterTreeStatistics(const ClusterTreeBase<Derived> &CT,
  ...
  std::cout << "cluster splitting method:     "
            << RRCA::internal::traits<Derived>::Splitter::splitterName()
            << std::endl;
            ...
            << "mean cluster discrepancy:     " << mean_disc << std::endl;
  {
    ...
    RRCA::Index intervals = 15;
    RRCA::Scalar h = (max_min_dist - min_min_dist) / intervals;
    RRCA::Vector values(intervals);
    values.setZero();
    ...
    RRCA::Scalar bar_factor = 40 * min_dist.size() / values.maxCoeff();
...

  {
    ...
    RRCA::Index intervals = 15;
    RRCA::Scalar h = log(max_disc / min_disc) / intervals;
    RRCA::Vector values(intervals);
    values.setZero();
    ...
    RRCA::Scalar bar_factor = 40 * disc_vec.size() / values.maxCoeff();
    bar_factor = bar_factor < Scalar(1. / 0.) ? bar_factor : 0;
    for (auto i = 0; i < intervals; ++i) {
      std::cout << std::scientific << std::setprecision(2) << std::setw(9)
